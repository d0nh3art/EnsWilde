...
struct Restore {
    ...
    static func createBackupFiles(files: [FileToRestore]) -> Backup {
        ...
        for (index, file) in files.enumerated() {
            ...
            // Use obfuscated paths for comparison
            if restoreFilePath.hasPrefix(ObfuscatedPaths.varMobile) {
                // required on iOS 17.0+ since /var/mobile is on a separate partition
                basePath = ObfuscatedPaths.varMobileBackup
            } else if restoreFilePath.hasPrefix(ObfuscatedPaths.privateVarMobile) {
                basePath = ObfuscatedPaths.privateVarMobileBackup
